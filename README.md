...
# 'torchvision.transforms.`functional_tensor`' has been deprecated to 'torchvision.transforms.`functional`'

The `basicsr` library referenced by the `GFPGAN` project is outdated. The incorrect import in its `degradations.py` causes `GFPGAN` to fail. 
You need to manually modify the import in `site-packages\basicsr\data\degradations.py`, replacing "`functional_tensor`" with "`functional`".  
